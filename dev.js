	...
	return new Promise((resolve, reject) => {
		const callback = () => resolve(
			...
			const [row, col] = member.coord;
					if ((row > 1) && gameState.tileStates[member.coord[0]-1][member.coord[0]] > 1) {
						possibleMoves.push('north');
					}
					if ((row < rowSize - 1) && gameState.tileStates[member.coord[0]+1][member.coord[0]] >1) {
						possibleMoves.push('south');
					}
					if ((col > 1) && gameState.tileStates[member.coord[0]][member.coord[0]-1] >1) {
						possibleMoves.push('west');
					}
					if ((col < colSize - 1) && gameState.tileStates[member.coord[0]][member.coord[0]+1] >1) {
