	...
	*/
	...
	for (i = 0; i < 3; i++){
		for (j = 0; j < 3; j++){
			...
			console.log(Math.abs(Math.abs(teams[0][i][0]-teams[1][j][0])-1) + Math.abs(Math.abs(teams[0][i][1]-teams[1][j][1])-1));
			val -= (Math.abs(Math.abs(teams[0][i][0]-teams[1][j][0])-1) + Math.abs(Math.abs(teams[0][i][1]-teams[1][j][1])-1));
		}
